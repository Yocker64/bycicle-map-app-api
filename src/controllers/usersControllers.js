...
module.exports = {
  ...
  getUserPosts: async (req, res) => {
    ...
    try {
      const posts = await postsQueries.getUserPosts(
        id,
        page,
        limit,
        sortField,
  ...
  getUserDrafts: async (req, res) => {
    ...
    const sortOrder = req.query.sortOrder || 'desc';
    try {
      const drafts = await postsQueries.getUserDrafts(
        id,
        page,
        limit,
        sortField,
  ...
  getUserLikedPosts: async (req, res) => {
    ...
    const sortOrder = req.query.sortOrder || 'desc';
    try {
      const posts = await postsQueries.getUserLikedPosts(
        id,
        page,
        limit,
        sortField,
  ...
  getUserCommentedPosts: async (req, res) => {
    ...
    const sortOrder = req.query.sortOrder || 'desc';
    try {
      const posts = await postsQueries.getUserCommentedPosts(
        id,
        page,
        limit,
        sortField,
  ...
  loggedUserUnfollow: async (req, res) => {
    ...
    const followingId = req.params.id;
    try {
      const unfollow = await followsQueries.removeFollow(
        followerId,
        followingId,
      );
      res.status(200).json({
