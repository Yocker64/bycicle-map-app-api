...
module.exports = {
  ...
  updateCommentContent: async (req, res) => {
    ...
    try {
      const comment = await commentsQueries.updateCommentContent(
        id,
        commentContent,
      );
      res.status(200).json({
  ...
  getUsersWhoLikedComment: async (req, res) => {
    ...
    try {
      const users = await usersQueries.getUsersWhoLikedComment(
        commentId,
        page,
        limit,
      );
  ...
  getNestedComments: async (req, res) => {
    ...
    try {
      const nestedComments = await commentsQueries.getNestedComments(
        id,
        page,
        limit,
        sortField,
  ...
  addNestedComment: async (req, res) => {
    ...
    try {
      const nestedComment = await commentsQueries.addNestedComment(
        userId,
        postId,
        comment,
        parentId,
      ...
      // Create Notification
      notificationQueries.createCommentReplyNotification(
        nestedComment.parent.userId,
        userId,
        parentId,
      );
    } catch (error) {
  ...
  loggedUserLikeComment: async (req, res) => {
    ...
    const commentId = req.params.id;
    try {
      const commentLike = await commentLikesQueries.addCommentLike(
        userId,
        commentId,
      );
      res.status(201).json({
      ...
      // Create Notification
      notificationQueries.createCommentLikeNotification(
        commentLike.comment.userId,
        userId,
        commentId,
      );
    } catch (error) {
  ...
  loggedUserUnlikeComment: async (req, res) => {
    ...
    const commentId = req.params.id;
    try {
      const commentLike = await commentLikesQueries.removeCommentLike(
        userId,
        commentId,
      );
      res.status(200).json({
