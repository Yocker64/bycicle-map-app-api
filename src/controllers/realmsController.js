...
module.exports = {
  ...
  createRealm: async (req, res) => {
    ...
    try {
      ...
      const existingRealm = await realmsQueries.existRealm(
        'name',
        req.body.name,
      );
      if (existingRealm) {
      ...
      const realm = await realmsQueries.createRealm(
        creatorId,
        name,
        description,
      );
  ...
  getRealmPosts: async (req, res) => {
    ...
    try {
      const posts = await postsQueries.getRealmPosts(
        id,
        page,
        limit,
        sortField,
  ...
  loggedUserJoinRealm: async (req, res) => {
    ...
    try {
      ...
      // Create Notification
      notificationQueries.createRealmJoinNotification(
        join.realm.creatorId,
        userId,
        realmId,
      );
    } catch (error) {
