...
module.exports = {
  ...
  getFeed: async (req, res) => {
    ...
    try {
      ...
      const followingUserIds = (
        await usersQueries.getUserFollowing(userId)
      ).map((user) => user.id);
      const joinedRealmIds = (
        await realmsQueries.getUserJoinedRealms(userId)
      ).map((realm) => realm.id);
      // Get posts from user and realm Ids
      const posts = await postsQueries.getFeed(
        followingUserIds,
        joinedRealmIds,
        page,
        limit,
  ...
  getPostRootComments: async (req, res) => {
    ...
    const sortOrder = req.query.sortOrder || 'desc';
    try {
      const comments = await commentsQueries.getPostRootComments(
        id,
        page,
        limit,
        sortField,
  ...
  loggedUserLike: async (req, res) => {
    ...
    try {
      ...
      // Create Notification
      notificationQueries.createPostLikeNotification(
        like.post.authorId,
        userId,
        postId,
      );
  ...
  loggedUserAddComment: async (req, res) => {
    ...
    try {
      // create root comment
      const comment = await commentsQueries.addRootComment(
        userId,
        postId,
        commentContent,
      );
      ...
      // Create Notification
      notificationQueries.createPostCommentNotification(
